...
function checkFurikae(year, month, day) {
  let dayOfWeek = new Date(year, month - 1, day).getDay();  // 曜日
  let furikaeFlg = false;

  // 月曜
  if (dayOfWeek == 1) {
    furikaeFlg = checkHoliday(year, month, day - 1);
  }
  // 火曜日
  if (dayOfWeek == 2 && furikaeFlg != true) {
    furikaeFlg = checkHoliday(year, month, day - 1)
                      && checkHoliday(year, month, day -2);
  }
  // 水曜日
  if (dayOfWeek == 3 && furikaeFlg != true) {
    furikaeFlg = checkHoliday(year, month, day - 1)
                      && checkHoliday(year, month, day - 2)
                      && checkHoliday(year, month, day - 3);

  // 祝日と祝日に挟まれているかどうか(国民の祝日)
  if ((dayOfWeek != 0 && dayOfWeek != 6) && furikaeFlg != true) {
    ...
    furikaeFlg = checkHoliday(prevDate.getUTCFullYear()
                                          , prevDate.getMonth() + 1
                                          ,prevDate.getDate())
